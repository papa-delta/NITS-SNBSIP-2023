// Do NOT change the order of the preprocessor directives below, having the order below is crucial!
...
void insertion_canonical_without_filewrite(char fname[5][100], int kmer_len, int threshold, int k) {
    ...
    char cmd[150] = "wc -c <", *s1 = " > linecount.txt";  // Do not change the value of the string, this is the system command to be run
    strcat(cmd, fname[0]);
    strcat(cmd, s1);
